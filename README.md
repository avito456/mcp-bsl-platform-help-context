...
| `info` | Детальная информация о конкретном элементе API по точному имени | `name` (точное имя, например `НайтиПоСсылкам`), `type` (method/property/type) |
...
7. **Режимы поиска**:
   - `keyword` — быстрый, для точных имён API (НайтиПоСсылкам, ValueTable)
